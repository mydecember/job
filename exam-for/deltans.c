...
//printf("%ld,%ld\n",p->tv_sec,p->tv_nsec);
	...
	//printf("(42.0+len)*n=%f\n",(42.0+len)*n);
	printf("packet per sec:%0.1f\nspeed:%0.1fMbps\n",n*1.0/p->tv_sec,(((len+24)*n*1.0)/(tns))*8);
}
void speed1(struct timespec *p, long long n,long long len)
{
//printf("%ld,%ld\n",p->tv_sec,p->tv_nsec);
	double tns=p->tv_sec*1000000+p->tv_nsec/1000;
	//printf("(42.0+len)*n=%f\n",(42.0+len)*n);
	printf("packet per sec:%0.1f\nspeed:%0.1fMbps\n",n*1.0/p->tv_sec,((len*1.0)/(tns))*8);
}
void speed2(struct timespec *p, long long n,long long len,long num,long delay)
{
	//printf("%d,%d\n",num,delay);
//printf("%ld,%ld\n",p->tv_sec,p->tv_nsec);
	//printf("%d\n",num*delay/1000);
	double tns=p->tv_sec*1000000+p->tv_nsec/1000-(int)num*1.0*delay/1000.0;
	//printf("(42.0+len)*n=%f\n",(42.0+len)*n);
	printf("packet per sec:%0.1f\nspeed:%0.1fMbps\n",n*1.0/p->tv_sec,((len*1.0)/(tns))*8);
}
