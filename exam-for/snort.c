...
long long getpacknum = 0;
int exitflag = 0;
#define SERVER_PORT 3333
int sockfd;
long long losepacket=0L;
long getpackt=0L;


#define QUEUE_NODE_LEN 3500
#define QUEUE_NODE_NUM (1<<13)
#define QUEUE_LEN_MASK (QUEUE_NODE_NUM-1)
struct classify{	
		char **data;
...
struct classify *g_crc_queue ;

struct classify * init_crc_queue()
{
	struct classify * qc = NULL;

	qc = (struct classify *)malloc(sizeof(struct classify ));
	int size = sizeof(char);
	int point_size = sizeof(char*);

	qc->data = (char **)malloc(point_size * QUEUE_NODE_NUM + size * QUEUE_NODE_LEN * QUEUE_NODE_NUM);
	if ((qc->data) == NULL)
	{
		printf("queue init error\n");
		return NULL;
	}
	memset(qc->data, 0, point_size * QUEUE_NODE_NUM + size * QUEUE_NODE_LEN * QUEUE_NODE_NUM);
	char * head = (char*)((unsigned long)qc->data + point_size * QUEUE_NODE_NUM);
	int row = QUEUE_NODE_NUM;
	while (row--)
		(qc->data)[row] = (char*)((unsigned long)head + row*QUEUE_NODE_LEN*size);
	/*int i = 0;
	for (; i < QUEUE_NODE_NUM; ++i)
	{
		qc[i].data = (char*) malloc(sizeof(char)*QUEUE_NODE_LEN);
		if (qc[i].data == NULL)
			return NULL;
		qc[i].head=0;
		qc[i].tail=0;
	...
	qc->head=0;
	qc->tail=0;
	pthread_mutex_init(&qc->work_mutex,NULL);
	qc->efd = eventfd(0, EFD_NONBLOCK);
	if (qc->efd < 0)
    {
       printf("EIeventfd failed.\n");
       return NULL;
    }
	return qc;
...
void destroy_queue(struct classify * qe)
{
	if (qe!=NULL)
	{
		free(qe->data);
		pthread_mutex_destroy(&qe->work_mutex);
	}
	close(qe->efd);
...
void threadpro(void)
{
	...
	FILE* file_log = init_log("/run/shm/snort_thread.log");

	if((host=gethostbyname("10.0.0.2"))==NULL) {
    ...
    read_event.data.fd = sockfd;
	

    ret = epoll_ctl(ep_fd, EPOLL_CTL_ADD, sockfd, &read_event);
    if (ret < 0)
    {
            ...
            for (i=0 ; i < ret; i++)
            {
               // printf("%d\n",i);
                ...
                else if (events[i].events & EPOLLIN)
                {
						...
						if (!headflag)
						{
							headrcv=recv(events[i].data.fd,&pknum,4,0);
							//printf("fd=%d, ret=%d, pknum=%d\n", events[i].data.fd, headrcv, pknum);
						 	if(headrcv != 4)
						 	{
						 		printf("recv error, %d, headrcv=%d\n",__LINE__, headrcv);
						 		goto fail;
						 	}
							memcpy(buf, &pknum, 4);
							headflag=1;
						...
						{
							datarcv=recv(events[i].data.fd,&buf[haverecv],pknum-haverecv,0);
							//printf("events[i].data.fd=%d,datarcv=%d, pknum=%d\n", events[i].data.fd, datarcv, pknum);
					 		if(datarcv <= 0)
					 		{
					 			perror("datarcv\n");
					 			printf("recv error, %d\n",__LINE__);
					 		...
					 		if (pknum == haverecv)
					 		{								
								pthread_mutex_lock(&g_crc_queue->work_mutex);
								if (((g_crc_queue->tail + 1)&QUEUE_LEN_MASK) != g_crc_queue->head )
								{									
									memcpy(g_crc_queue->data[g_crc_queue->tail], buf, pknum);
									g_crc_queue->tail++;
									getpackt++;
									//fprintf(file_log,"getpackt=%ld\n", getpackt);
									printf("getpackt=%d\n", getpackt);
								}
								else
								{
									losepacket++;
									//fprintf(file_log,"losepacket=%ld\n", losepacket);
								}
								pthread_mutex_unlock(&g_crc_queue->work_mutex);
								if (write(g_crc_queue->efd, &count, 8) < 0)
							    {
							        perror("write event fd fail:\n");
							        printf("g_crc_queue->efd=%d\n",g_crc_queue->efd);
							       // return;
							    }
								headflag=0;
								haverecv=0;								 
							...
							else if (pknum < haverecv)
							{
								fprintf(file_log, "recv error, %d, pknum=%d, haverecv=%d\n",__LINE__, pknum, haverecv);
								//fprintf(file_log,"losepacket=%ld\n", losepacket);
							}
...
int SnortMain(int argc, char *argv[])
{
	...
	*******************/
	...
	g_crc_queue = init_crc_queue();
	if (g_crc_queue == NULL)
		goto end;
	pthread_t productid;
	int ret=pthread_create(&productid,NULL,(void *) threadpro,NULL);
	if(ret!=0){
	...
	add by zhaofuqiang
	*******************/
	pthread_join(productid,NULL);
	destroy_queue(g_crc_queue);
...
void PacketLoop (void)
{
    ...
    read_event.events = EPOLLHUP | EPOLLERR | EPOLLIN;//EPOLLET;
    read_event.data.fd = g_crc_queue->efd;

    ret = epoll_ctl(ep_fd, EPOLL_CTL_ADD, g_crc_queue->efd, &read_event);
    if (ret < 0)
    {
        perror("epoll ctl failed:");
        goto fail;
	...
	FILE* file_log = init_log("/run/shm/snort_loop.log");
	fprintf(file_log, "loop start. %d:%s\n",errno, strerror(errno));
	 
    while ( !exit_logged )
    {
    	/****************************************
    		modify by zhaofuqiang
    		***********************************/
       ...
       while (!exitflag)
            ...
            for (i=0 ; i < ret; i++)
            {
               // printf("%d\n",i);
                if (events[i].events & EPOLLHUP)
                {
					...
					fprintf(file_log, "epoll eventfd has epoll hup. %d:%s\n",errno, strerror(errno));
                   // goto fail;
                }
                else if (events[i].events & EPOLLERR)
                {
                   // perror("epoll eventfd has epoll error.\n");
                   fprintf(file_log, "epoll eventfd has epoll error. %d:%s\n",errno, strerror(errno));
                    goto fail;
                }
                else if (events[i].events & EPOLLIN)
                {
				  ...
				  // printf("read res=%d, count=%d\n", res, count);
				//  fprintf(file_log, "read res=%d, count=%d\n", res, count);
                    ...
                    {
                        //perror("read fail:");
                        fprintf(file_log, "read fail:. %d:%s\n",errno, strerror(errno));
                        goto fail;
                    }
                    else
                    {                  	             	
                    	...
                    	for(counti=0; counti<count; ++counti)                    	
                    	{
                    		...
                    		/*if ((g_crc_queue->head) != g_crc_queue->tail )
							{*/									
								
								memcpy(&bufnum, g_crc_queue->data[g_crc_queue->head], 4);
								memcpy(data, g_crc_queue->data[g_crc_queue->head]+4,bufnum);
								
								pthread_mutex_lock(&g_crc_queue->work_mutex);
								g_crc_queue->head++;
								pthread_mutex_unlock(&g_crc_queue->work_mutex);
								
								 hdr.caplen = bufnum;//pkth->caplen;    
								 hdr.pktlen = bufnum;//pkth->len;    
								// hdr.ts = shmp->ts[shmp->head];//pkth->ts;    
        ...
        else if (ret == 0)
        {
        	...
        	{
        		//perror("thread exit\n");
				 fprintf(file_log, "thread exit ret=0: %d:%s\n",errno, strerror(errno));
        		goto fail;
        	}
            /* time out */
           // msg("epoll wait timed out. thread_id=%d\n",thread_id);
