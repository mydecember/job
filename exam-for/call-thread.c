...
volatile struct bc_class bcbuf;
sem_t bin_sem;
void initBCBuff()
{
...
//write(mypipe[1],&i,1);
      	...
      	else if(temp!=NULL)
      	{
			...
			p->ptcp=(unsigned char*)(p->buf)+(tcp->th_off*4)+((unsigned char*)tcp-(unsigned char*)mac);//ptcp;
      			...
      			if(i==bcbuf.head)
      				{
      				//	msg("bcbuf.head=%d,bcbuf.tail=%d\n",bcbuf.head,bcbuf.tail);
      					losepacket++;
      				}
      				else
      					{
						...
						//msg("nnnnnnnnn\n");
						//write(mypipe[1],&i,1);
      					//	if((bcbuf.tail-bcbuf.head+BC_CLASS_NUM)%BC_CLASS_NUM==100)
      					//sem_getvalue(&bin_sem,&semnum);
      					//if(semnum==0)
      						//sem_post(&bin_sem);      				
      					}
				...
				//temp->proto=i;
      		//		msg(" %s\n",pro_map[i]);
				/*if(i==POP3||i==SMTP||i==FTP)
				{
					//msg("hhhhhhhhhhhhh\n");
					int fd=open("/home/zhao/pop3_log.txt",O_CREAT|O_APPEND|O_RDWR,0666);
					if(fd<0)
						msg("open error\n");
					p=temp->bc_head;
	      				while(p!=NULL)
	      				{
						write(fd,p->ptcp,p->tcplen);						
	      					p=p->next;
	      				}
...
if(pipe(mypipe)<0)
	...
	int pipeflag;
printf("llllllll\n");	
pipeflag=fcntl(mypipe[1],F_GETFL,0);
	msg("ccccc\n");
	pipeflag|=O_NONBLOCK;
	if(fcntl(mypipe[1],F_SETFL,pipeflag)<0)
	{
		msg("fcntl error\n");
		exit(0);
	}
