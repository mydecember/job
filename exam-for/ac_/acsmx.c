...
*/ 
int acsmAddPattern (ACSM_STRUCT * p, unsigned char *pat, int n, int nocase,int pro) 
{

	ACSM_PATTERN * mlist = p->acsmPatterns;
	//printf("\n### Summary ###\n");
	for (;mlist!=NULL;mlist=mlist->next)
	{
		//printf("--%s\n",mlist->patrn);
		if(!memcmp(pat,mlist->casepatrn,n)||!memcmp(pat,mlist->patrn,n))
		{
			int i;
			for(i=0;i<mlist->pro_num;++i)
			{
				if(mlist->feature_id[i]==pro)
				{
					return 0;
				}
			}			
			mlist->feature_id[mlist->pro_num]=pro;
			mlist->pro_num++;
			printf("aaaaaaaa\n");
			return 0;
		}	
	...
	plist->pro_num=0;
	plist->feature_id[plist->pro_num]=pro;
	plist->pro_num++;
	...
	for (state = 0; T < Tend; T++)
	{
		...
		if( StateTable[state].MatchList != NULL )
		{
				...
				mlist=mlist->next )
			{
				...
				if (!memcmp(temp->patrn,mlist->patrn,temp->n)) //strcmp succeed return 0,So here use "!" operation
						{
							temp->nmatch++;
						}
...
PRO getSummary (ACSM_PATTERN * pattern,int *array)
{	
	ACSM_PATTERN * mlist = pattern;
	...
	for (;mlist!=NULL;mlist=mlist->next)
	{
		...
		if(mlist->nmatch)
		{
			mlist->nmatch=0;
			for(i=0;i<mlist->pro_num;++i)
			find_num[mlist->feature_id[i]]++;
		}
	}
	for(i=0;i<PRO_MAX;++i)
	{
		if(find_num[i]==array[i])
			return i;
	}
	return UNKOWN;
}
